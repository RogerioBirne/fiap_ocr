...
def convert_image_perspective(img):
    return __convert_image_perspective(img, image_color_filter.color_otsu_threshold)


def __convert_image_perspective(img, color_threshold_filter):
    img_edge = image_color_filter.image_to_canny_edge(img, color_threshold_filter)
    ...
    if fiscal_ticket_contour is not None:
        ...
        if __check_is_valid_contour(img, fiscal_ticket_contour):
...
def __check_is_valid_contour(img, fiscal_ticket_contour):
    height = img.shape[0]
    min_height = int(height * 0.60)
    ...
    return dist_y1 > min_height and dist_y2 > min_height
